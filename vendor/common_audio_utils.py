...
class MyPrinter:
    """In-place streaming printer — overwrites current line during active speech.
    Also writes to a status file for the subtitle window display."""
    def __init__(self, status_file=None):
        self.last_len = 0
        self.first_print = True
        self.status_file = status_file
        self._completed_lines = []  # history of finalized utterances
        self._max_history = 50

    def do_print(self, result):
        if not result:
            return
        # Overwrite current line with carriage return (no newline)
        clear = ' ' * max(self.last_len - len(result), 0)
        print(f'\r{result}{clear}', end='', flush=True)
        self.last_len = len(result)
        self.first_print = False

        # Write to status file: all completed lines + current in-progress
        if self.status_file:
            try:
                lines = self._completed_lines + [result]
    ...
    def on_endpoint(self):
        # Finalize current line, save to history
        print('', flush=True)
        if self.status_file and self.last_len > 0:
            # We don't know the exact text that was last printed,
            # but the next do_print will start fresh.
            pass
        self.last_len = 0
        self.first_print = True

    def on_sentence_final(self, final_text):
        """Called when VAD marks a complete segment — commit to history."""
        if final_text.strip():
            self._completed_lines.append(final_text.strip())
            if len(self._completed_lines) > self._max_history:
